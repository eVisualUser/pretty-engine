...
public:
  ...
  void Update(WorldManager *worldManager, Input *input, Renderer *renderer, PhysicalSpace *physicalSpace, bool* isEditor) {
    ...
    if (ImGui::Begin("Play Mode")) {
      ...
      if (!*isEditor && ImGui::Button("Stop")) {
          renderer->Clear();
    // todo: allow to stop playing
          worldManager->ClearWorldInstances();
          worldManager->ActiveAllWorldsFiles();
